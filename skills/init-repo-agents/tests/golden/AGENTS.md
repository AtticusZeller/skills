...
# Example Project · Agent Collaboration Guide

> Exercises deterministic repository scaffolding.

**Primary toolchain:** Python 3.12 + uv/ruff/ty
...
- [[docs/core.md]] — core module (`src/example/core/`)
- [[docs/io.md]] — io module (`src/example/io/`)
...
- **`dev.sh` (conventionally `dev.sh`)** wraps common development and launch commands: environment setup, lint, format, typecheck, tests, training, inference, evaluation, and data processing. Expose them as subcommands such as `./dev.sh train`, `./dev.sh eval`, and `./dev.sh lint`.
