...
- Keep generated or local-machine artifacts out of the repo, except deterministic
  upstream mirror outputs maintained by a checked-in sync script.
- If adding an external skill dependency, update `manifests/global-skills.json`
  instead of vendoring it unless the user explicitly approves a maintained mirror.
